...
### **3. Yoast SEO + WPGraphQL Integration (Technical Visibility)**
- **Concept**: Synchronize enterprise-grade SEO metadata from the WordPress Yoast plugin directly into the Next.js `generateMetadata` API.
- **Implementation Plan**:
    1. **WordPress Side**: Install and configure the `wp-graphql-yoast` extension to expose SEO fields in the GraphQL schema.
    2. **Schema Extension**: Update `types/wordpress.ts` with a `WPSEO` interface (title, metaDesc, canonical, focuskw, opengraphTitle, etc.).
    3. **Fragment Update**: Enhance `RESOURCE_CARD_FIELDS` and single resource queries in `lib/graphql/fragments.ts` to fetch the `seo` object.
    4. **Dynamic Metadata**: Refactor `app/resources/[slug]/page.tsx` to map the incoming GraphQL SEO data to the Next.js `Metadata` object.
    5. **Benefit**: Full SEO parity with traditional WordPress themes while maintaining the performance of a headless frontend.
